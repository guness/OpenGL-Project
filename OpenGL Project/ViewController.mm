...
- (void)viewDidLoad {
    ...
    [self.wireframeCheck setState:self.openGLView.SHOW_WIREFRAME?NSOnState:NSOffState];
    [self.showControlPointsCheck setState:self.openGLView.SHOW_CONTROL_POINTS?NSOnState:NSOffState];
    ...
    [self.transparencyText setIntValue:self.openGLView.color.alphaComponent * 100];
    [self.transparencySlider setIntValue:self.openGLView.color.alphaComponent * 100];
}
...
- (IBAction)transparencyChanged:(id)sender {
    ...
    self.openGLView.color = [self.openGLView.color colorWithAlphaComponent:val/100.0f];
}
    ...
    [panel beginSheetModalForWindow:window completionHandler:^(NSInteger result){
        ...
        if (result == NSFileHandlingPanelOKButton) {
            for (NSURL *url in [panel URLs]) {
                NSImage *image = [[NSImage alloc] initWithData:[NSData dataWithContentsOfURL:url]];
