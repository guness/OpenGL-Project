...
/* This is used like constructor. OSX calls this method for initialization of MyOpenGLView  */
- (id)initWithCoder:(id)coder {
    ...
    if (self) {
        ...
        lastX=0;
        lastY=0;
        self.color = [NSColor colorWithCalibratedRed:0.3f green:0.6f blue:1.0f alpha:0.6f];
    }
    return self;
}
...
/* We dont use glut's idle method. Instead, we are creating a timer in order to get a fixed FPS. */
- (void) initUpdateTimer {
    timer = [NSTimer timerWithTimeInterval:kScheduledTimerInSeconds target:self selector:@selector(heartbeat) userInfo:nil repeats:YES];
...
-(void) drawRect: (NSRect) theRect {
    [[self openGLContext] makeCurrentContext];      //Thread related, OSX's internal bussiness.
    [self resizeView];          //Calculating viewport according to changes on View's size.
    ...
    glSwapAPPLE();  //equivalent to glutSwapBuffers() or [[self openGLContext] flushBuffer];
    
}

/* This method is used for sending all data to shaders using uniform location variables.  */
- (void) sendDataToShaders {
    ...
    if(self.UPDATE_NEEDED){
        ...
        if(!self.EDIT_MODE && SURFACE_DATA_SIZE>2){
            ...
            glBufferSubData(GL_ARRAY_BUFFER, sizeof(cpoints)+ sizeof(points)+ sizeof(cnormals), sizeof (normals[0]) * SURFACE_DATA_SIZE, normals);
        }
    }
...
- (void) updatePerspectiveMatrix {
    ...
    mat4 projection = Perspective(self.zoom, aspect, 0.0001, 100.0); //set the perspective (angle of sight, width, height, ,
    glUniformMatrix4fv(Projection, 1, GL_TRUE, projection);
} // updatePrespectiveMatrix

/* Handles resizing/updating of OpenGL needs context update and if the window dimensions change, window dimensions update, reseting of viewport and an update of the projection matrix. */
- (void) resizeView {
...
- (void)camera {
    ...
    Translate(-self.xpos, -self.ypos, -self.zpos); //translate the screen to the position of our camera
}
- (void) heartbeat {
    [self drawRect:[self bounds]];
}
/* This is called when OpenGLView is about be shown. We are setting OpenGL version 3.2 compatibility here. */
- (void) awakeFromNib
{
    NSOpenGLPixelFormatAttribute attributes [] = {
        NSOpenGLPFADepthSize, (NSOpenGLPixelFormatAttribute)24,
        NSOpenGLPFAOpenGLProfile, NSOpenGLProfileVersion3_2Core,NSOpenGLPFADoubleBuffer,NSOpenGLPFAAccelerated,
...
/* glut' keyboard convenient. But this method is being called before calculating ModelView for each frame. If there is no key pressed it's function is NOP. */
- (void)keyboard{
...
/* We are getting reference in order to update GUI element's values after a change on values like position, rotation etc. */
